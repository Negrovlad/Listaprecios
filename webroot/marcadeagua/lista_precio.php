<?php 	
	...
	// Función para determinar el nombre del archivo a utilizar para la lista de precios
	...
	function Header()
	{
		...
		// verificar y tomar parámetro del nombre del cliente
		$nombre_cliente = $_POST['nombre_usuario'];
		
		$texto_marca_de_agua = $nombre_cliente;
		
		// de acuerdo con el largo del texto, calcular un tamaño de fuente y una posición X
		...
		if ($largo_texto < 20){
			...
			// 80 se considera la mitad, de acuerdo con el tamaño de letra
			...
			$this->SetTextColor(100,175,228);
			// Calcular la coordenada X para que quede centrado el nombre del cliente
			// 60 se considera la mitad, de acuerdo con el tamaño de letra
			$posx = 60 - round($largo_texto/2);

		}
			

		// Coordenadas X, Y, Texto y ángulo de rotación
	...
	if(!empty($_POST)){
		...
		for ($num_pagina = 1; $num_pagina <= $total_paginas; $num_pagina++) {	
				...
				// agragar una nueva página
				$pdf->AddPage(); 				
				
				// utilizar la página importada y colocarla en la 0,0
				// Ancho y alto, el último parámetro es si se ajustael tamaño de la página 			
